...
SELECT SUM(O.SALEPRICE) FROM ORDERS O JOIN CUSTOMER C ON O.CUSTOMERID = C.CUSTOMERID WHERE C.NAME = '박지성';
--9. 박지성의 구매한 도서 수를 구하시오.
SELECT COUNT(*) FROM ORDERS WHERE CUSTOMERID = (SELECT CUSTOMERID FROM CUSTOMER WHERE NAME = '박지성');
SELECT COUNT(*) FROM ORDERS O JOIN CUSTOMER C ON O.CUSTOMERID = C.CUSTOMERID WHERE C.NAME = '박지성';
